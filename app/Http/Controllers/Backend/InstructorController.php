...
class InstructorController extends Controller
{
    ...
    {
        $row = new InstructorResource(Instructor::findOrFail(decrypt($id)));
        return response()->json(['row' => $row], 200);
    }

    public function update(MemberUpdateRequest $request, $id)
    {
        $row = Instructor::createOrUpdate(decrypt($id), $request->all());
        if($row === true) {
    ...
    {
        $data = Instructor::where(['status' => true, 'trash' => false]);
        ...
        return response()->json(['rows' => InstructorResource::collection($data)], 200);
    }
}
