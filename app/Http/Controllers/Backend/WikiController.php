...
class WikiController extends Controller
{
    ...
    public function index()
    {
        ...
        return response()->json([
            ...
            'permissions' => $this->permissions('wikis'),
            'rows'        => $rows,
    ...
    {
        $row = Wiki::createOrUpdate(NULL, $request->all());
        if($row === true) {
    ...
    {
        $row = new WikiResource(Wiki::has('tenant')->findOrFail($id));
        return response()->json(['row' => $row], 200);
    }

    public function update(WikiUpdateRequest $request, $id)
    {
        $row = Wiki::createOrUpdate($id, $request->all());
        if($row === true) {
    ...
    {
        $data = Wiki::has('tenant')->where(['status' => true, 'trash' => false]);
        ...
        return response()->json(['rows' => WikiResource::collection($data)], 200);
    }
}
