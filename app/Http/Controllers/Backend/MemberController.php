...
class MemberController extends Controller
{
    ...
    {
        $row = new MemberResource(Member::findOrFail(decrypt($id)));
        return response()->json(['row' => $row], 200);
    }

    public function update(MemberUpdateRequest $request, $id)
    {
        $row = Member::createOrUpdate(decrypt($id), $request->all());
        if($row === true) {
    ...
    {
        $data = Member::where(['status' => true, 'trash' => false]);
        ...
        return response()->json(['rows' => MemberResource::collection($data)], 200);
    }
}
