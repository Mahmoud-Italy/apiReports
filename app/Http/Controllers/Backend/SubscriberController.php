...
class SubscriberController extends Controller
{
    ...
    {
        $data = Subscriber::get();
        $rows = SubscriberResource::collection(Subscriber::fetchData(request()->all()));
    ...
    {
        $row = Subscriber::createOrUpdate(NULL, $request->all());
        if($row === true) {
    ...
    {
        $row = new SubscriberResource(Subscriber::findOrFail(decrypt($id)));
        return response()->json(['row' => $row], 200);
    }

    public function update(SubscriberUpdateRequest $request, $id)
    {
        $row = Subscriber::createOrUpdate(decrypt($id), $request->all());
        if($row === true) {
    ...
    {
        $data = Subscriber::where(['status' => true, 'trash' => false]);
        ...
        return response()->json(['rows' => SubscriberResource::collection($data)], 200);
    }
}
