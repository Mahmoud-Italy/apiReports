...
class CategoryController extends Controller
{
    ...
    {
        $row = Category::createOrUpdate(NULL, $request->all());
        if($row === true) {
    ...
    {
        $row = Category::createOrUpdate($id, $request->all());
        if($row === true) {
    ...
    {
        $data = Category::has('tenant')->where(['status' => true, 'trash' => false]);
        ...
        return response()->json(['rows' => CategoryResource::collection($data)], 200);
    }
}
