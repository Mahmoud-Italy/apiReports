...
class OnlineController extends Controller
{
    ...
    {
        $data = OnlineTraining::get();
        $rows = OnlineTrainingResource::collection(OnlineTraining::fetchData(request()->all()));
        return response()->json([
            'statusBar'   => $this->statusBar($data),
            'rows'        => $rows,
            'active'      => Setting::select('status')->findOrFail(21),
            'paginate'    => $this->paginate($rows)
        ], 200);
    }

    public function store(OnlineTrainingStoreRequest $request)
    {
        $row = OnlineTraining::createOrUpdate(NULL, $request->all());
        if($row === true) {
    ...
    {
        $row = new OnlineTrainingResource(OnlineTraining::findOrFail(decrypt($id)));
        return response()->json(['row' => $row], 200);
    }

    public function update(OnlineTrainingUpdateRequest $request, $id)
    {
        $row = OnlineTraining::createOrUpdate(decrypt($id), $request->all());
        if($row === true) {
    ...
    {
        $data = OnlineTraining::where(['status' => true, 'trash' => false]);
        ...
        return response()->json(['rows' => OnlineTrainingResource::collection($data)], 200);
    }
}
